//! Statistical hypothesis testing, metrics reduction, and effect size calculation.
//!
//! Provides two-tailed Welch's t-test with variance regularization floor, Cohen's d effect size,
//! and complementary error function rational Chebyshev approximations.

/// Calculate the arithmetic sample mean of a slice of floats.
...
/// Calculate the sample standard deviation (unbiased estimator with N-1 denominator).
...
/// Rational Chebyshev approximation to complementary error function erfc(x) for x >= 0.
...
/// Returns a tuple of (t_statistic, p_value, cohen_d).
