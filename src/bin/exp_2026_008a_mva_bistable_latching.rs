//! Binary entry point for EXP-2026-008a: Bistable Resonant Latching and Nondestructive Dynamic Bit Storage.
...
fn print_help() {
    println!(
        r#"EXP-2026-008a: Bistable Resonant Latching and Nondestructive Dynamic Bit Storage
Milestone 2.1: Bistable Latching (Tier 2: Temporal Dynamics & State Retention)

USAGE:
    exp_2026_008a_mva_bistable_latching [OPTIONS]

OPTIONS:
    --help                    Display this help menu
    --output-dir <DIR>        Output directory for telemetry (default: data/telemetry/EXP-2026-008a)
    --seeds <N>               Number of seeds to evaluate per condition (default: 30)
    --delta-t <TICKS>         Quiescent retention horizon post-write (default: 1000)
    --t-warmup <TICKS>        Warmup settling window ticks (default: 50)
    --condition <COND>        Filter condition (active_latch, baseline_feedforward_loss, etc.)
    --noise-rate <EPS>        Filter background noise rate (0.00, 0.01, 0.05)
    --fast-check              Execute quick smoke test (1 seed, 100 delta_t, 20 warmup)
"#
    );
}
...
fn main() {
    ...
    let mut output_dir = PathBuf::from("data/telemetry/EXP-2026-008a");
    let mut num_seeds = 30;
    let mut delta_t: Option<usize> = None;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--delta-t" => {
                if i + 1 < args.len() {
                    delta_t = Some(args[i + 1].parse().unwrap_or(1000));
                    i += 1;
                }
            }
            "--t-warmup" => {
    ...
    println!("================================================================================");
    println!("EXP-2026-008a: Bistable Resonant Latching & Nondestructive Dynamic Bit Storage");
    println!("Milestone 2.1: Bistable Latching (Dynamic Bit Storage)");
