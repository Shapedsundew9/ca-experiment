...
$$\mathcal{N}_i = \left\{ j \in \mathcal{V} \mid (u_j, v_j) \in \{((u_i \pm 1) \bmod 4, v_i), (u_i, (v_i \pm 1) \bmod 4)\} \right\}$$
...
- $\mathbf{s}(t) = [s_0(t), s_1(t), \dots, s_{N-1}(t)]^T \in \{0, 1\}^N$: Instantaneous binary firing state vector.
- $\mathbf{V}(t) = [V_0(t), V_1(t), \dots, V_{N-1}(t)]^T \in \mathbb{R}_{\ge 0}^N$: Internal membrane charge accumulators.
- $\mathbf{R}(t) = [R_0(t), R_1(t), \dots, R_{N-1}(t)]^T \in \{0, 1, \dots, N_{\text{ref}}\}^N$: Integer refractory down-counters.
...
A discrete binary stream $\{x_t\}_{t \ge 0}$ with $x_t \in \{0, 1\}$ is injected at sensory port node $\iota_{\text{sensory}} = 0$.
...
An input coupling scalar $k_{\text{in}} \in \{0, 1\}$ gates the ingress:
$$u_i(t) = k_{\text{in}} \cdot x_t \cdot \mathbb{I}(i = \iota_{\text{sensory}})$$

### Linear Readout Layer

Let the observed instantaneous state vector at tick $t$ be $\mathbf{s}(t) \in \{0, 1\}^N$. The linear readout maps $\mathbf{s}(t)$ to an activation scalar $\hat{a}_t \in \mathbb{R}$ and a predicted binary decision $\hat{y}_t \in \{0, 1\}$:
...
1. **State Boundedness Invariant:** For all $t \ge 0$ and all $i \in \mathcal{V}$:
   $$s_i(t) \in \{0, 1\}, \quad R_i(t) \in \{0, \dots, N_{\text{ref}}\}, \quad \theta_i(t) \in [\theta_{\min}, \theta_{\max}]$$
2. **Deterministic Dynamical Trajectory:** Given initial conditions $\mathbf{S}(0)$ and input sequence $\{x_t\}_{t=0}^T$, the trajectory $\{\mathbf{S}(t)\}_{t=0}^T$ is strictly deterministic.
...
For a specified delay $\tau \in \mathbb{N}_{\ge 1}$, the target variable $y_t \in \{0, 1\}$ is defined by:
$$y_t = x_t \oplus x_{t-\tau} = x_t (1 - x_{t-\tau}) + (1 - x_t) x_{t-\tau} = x_t + x_{t-\tau} - 2 x_t x_{t-\tau}$$
In bipolar coordinates where $\tilde{x}_t = 2 x_t - 1 \in \{-1, +1\}$ and $\tilde{y}_t = 2 y_t - 1 \in \{-1, +1\}$:
