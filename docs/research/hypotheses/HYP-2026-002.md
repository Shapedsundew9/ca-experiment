...
- **State Space:**
  - **Graph Topology:** Discrete 2D torus $\mathbb{T}^2 = (\mathbb{Z}/4\mathbb{Z}) \times (\mathbb{Z}/4\mathbb{Z})$ containing $N = 16$ vertices $\mathcal{V} = \lbrace v_{(x, y)} \mid x, y \in \lbrace 0, 1, 2, 3 \rbrace \rbrace$.
  - **Connectivity:** Bidirectional 4-regular von Neumann neighborhood ($k_{\text{in}} = k_{\text{out}} = 4$):
    $$\mathcal{N}_{\text{in}}(x, y) = \lbrace (x \pm 1 \pmod 4, y), (x, y \pm 1 \pmod 4) \rbrace$$
  - **Sensory Ingress Nodes:** Designated subset $\mathcal{I}_{\text{sensory}} = \lbrace v_{(0, 0)}, v_{(0, 1)} \rbrace$ mapped to indices $\lbrace 0, 1 \rbrace$.
  - **Node State Vector:** At discrete clock tick $t \in \mathbb{N}$, node $i \in \mathcal{V}$ maintains state:
    ...
    where:
    - $s_i(t) \in \lbrace 0, 1 \rbrace$ is the instantaneous binary spike emission.
    - $R_i(t) \in \lbrace 0, 1, \dots, N_{\text{ref}} \rbrace$ is the refractory down-counter ($N_{\text{ref}} = 2$).
  ...
  - **Global State Snapshot:**
    ...
    with primary observable binary state configuration $\mathbf{s}(t) = (s_0(t), \dots, s_{N-1}(t)) \in \lbrace 0, 1 \rbrace^N$.

- **Update Operators:**
  1. **Input Aggregation Operator ($\mathcal{A}$):**
     $$A_i(t) = \sum_{j \in \mathcal{N}_{\text{in}}(i)} s_j(t-1) + \mathbb{I}_{\lbrace i \in \mathcal{I}_{\text{sensory}} \rbrace} u_i(t)$$
     where $u_i(t) \in \lbrace 0, 1 \rbrace$ denotes external driving input or neutral carrier clock.
...
- **Protocol Temporal Phases:**
  - **Drive Phase ($t \in [1, T_{\text{drive}}]$, $T_{\text{drive}} = 32$):**
    Nodes $i \in \mathcal{I}_{\text{sensory}}$ are clamped to input pattern bitstream $u_i(t) = \mathbf{u}_X(t)$ for $X \in \lbrace A, B, C, D \rbrace$.
...
- **Occupancy Distance ($D_{\text{occ}}$):**
  $$D_{\text{occ}}(S_A, S_B) = \frac{1}{N} \lVert \mathbf{m}_A - \mathbf{m}_B \rVert_1 = \frac{1}{N} \sum_{i=0}^{N-1} |m_{A, i} - m_{B, i}|$$
