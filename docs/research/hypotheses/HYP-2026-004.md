...
$$\mathcal{N}_i^{\text{in}} = \left\lbrace j \in \mathcal{V} \mid (u_j, v_j) \in \left\lbrace ((u_i \pm 1) \bmod 4, v_i), (u_i, (v_i \pm 1) \bmod 4) \right\rbrace \right\rbrace$$

with incoming degree $k_{\text{in}} = |\mathcal{N}_i^{\text{in}}| = 4$ for all $i \in \mathcal{V}$. Symmetrically, each node has four outgoing tracks $\mathcal{N}_i^{\text{out}}$. Non-adjacent node pairs are strictly uncoupled: $\mathcal{E} = \lbrace (j, i) \in \mathcal{V}^2 \mid j \in \mathcal{N}_i^{\text{in}} \rbrace$.
...
1. $\mathbf{s}(t) = [s_0(t), \dots, s_{N-1}(t)]^T \in \lbrace 0, 1 \rbrace^N$: Instantaneous binary firing state vector.
2. $\mathbf{V}(t) = [V_0(t), \dots, V_{N-1}(t)]^T \in \mathbb{R}_{\ge 0}^N$: Somatic membrane charge accumulators.
3. $\mathbf{R}(t) = [R_0(t), \dots, R_{N-1}(t)]^T \in \lbrace 0, 1, \dots, N_{\text{ref}} \rbrace^N$: Integer refractory down-counters ($N_{\text{ref}} = 2$).
...
Sensory ingress tracks inject external bitstreams into sensory ports $\mathcal{I}_{\text{sensory}} = \lbrace 0, 5 \rbrace$ (diagonal node pair to break toroidal spatial symmetry):
...
where $P \in \lbrace A, B, C, D \rbrace$ denotes a canonical 32-tick driving bitstream and $c(t)$ is the neutral alternating carrier clock preventing zero-input absorbing states during autonomous relaxation.
...
- $s_i(t+1) s_j(t) \in \lbrace 0, 1 \rbrace$ represents causal Long-Term Potentiation (LTP): presynaptic node $j$ fired at $t$, immediately preceding postsynaptic spike $i$ at $t+1$.
...
$$D_{\text{replay}}(P) = \frac{1}{N} \lVert \mathbf{m}_P^{(1)} - \mathbf{m}_P^{(2)} \rVert_1 = \frac{1}{N} \sum_{i=0}^{N-1} |m_{P, i}^{(1)} - m_{P, i}^{(2)}|$$

### Inter-Pattern Separation ($D_{\text{sep}}$)

For two distinct driving patterns $P \neq Q \in \lbrace A, B, C, D \rbrace$:

$$D_{\text{inter}}(P, Q) = \frac{1}{N} \lVert \mathbf{m}_P - \mathbf{m}_Q \rVert_1$$
...
Let $\mathbf{u}_P^\epsilon(t)$ be a perturbed version of driving bitstream $\mathbf{u}_P(t)$ where each bit is flipped independently with probability $\epsilon \in \lbrace 0.01, 0.05 \rbrace$. Let $\mathbf{m}_{P, \epsilon}$ be the settled occupancy vector under noisy driving, and $\mathbf{m}_{P, 0}$ be the unperturbed reference orbit.

The perturbation deviation distance is:

$$D_{\text{perturb}}(P, \epsilon) = \frac{1}{N} \lVert \mathbf{m}_{P, \epsilon} - \mathbf{m}_{P, 0} \rVert_1$$
