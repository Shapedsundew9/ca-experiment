...
- **State Space:**
  ...
  - Firing state: $\mathbf{s}(t) \in \{0, 1\}^N$, where $s_i(t) = 1$ denotes an action spike emitted by node $i$.
  - Refractory state: $\mathbf{r}(t) \in \{0, 1, \dots, N_{\text{ref}}\}^N$, with $N_{\text{ref}} = 2$.
...
- **Topology:**
  ...
  $$\mathcal{N}^{\text{in}}(i) = \{ (x, (y+1)\bmod 4), (x, (y-1)\bmod 4), ((x+1)\bmod 4, y), ((x-1)\bmod 4, y) \}$$
...
- **Parameters:**
  ...
  - Dendritic saturation limit: $W_{\text{sum\_max}} = 4.0$.
  - Metaplastic consolidation coefficient: $\kappa \in \{0.0, 1.0, 5.0\}$.
...
where $\xi_i(t) \in \{0, 1\}$ is the external sensory drive injected if node $i$ belongs to the active sensory ingress set $\mathcal{V}_{\text{sensory}}$, else $0$.
...
$$W_{ij}(t+1) = \begin{cases} \text{clip}\left( W_{ij}^*(t+1) \cdot \frac{W_{\text{sum\_max}}}{S_i(t+1)}, W_{\min}, W_{\max} \right) & \text{if } S_i(t+1) > W_{\text{sum\_max}} \\ \min\left( W_{\max}, W_{ij}^*(t+1) \right) & \text{otherwise} \end{cases}$$
...
1. **Dendritic Excitability Invariant:** Total incoming track weight per node is strictly bounded:
   $$\sum_{j \in \mathcal{N}^{\text{in}}(i)} W_{ij}(t) \le W_{\text{sum\_max}} = 4.0, \quad \forall i \in \mathcal{V}, \forall t \ge 0$$
...
- **Pattern $A$:** Ingress nodes $\mathcal{V}_A = \{0, 1\}$, periodic bit stream $P_A = [1, 0, 1, 0, 0, 1, 0, 0]$ (period 8).
- **Pattern $B$:** Ingress nodes $\mathcal{V}_B = \{4, 8\}$, periodic bit stream $P_B = [1, 1, 0, 0, 1, 0, 1, 0]$ (period 8).
- **Pattern $C$:** Ingress nodes $\mathcal{V}_C = \{2, 3\}$, periodic bit stream $P_C = [1, 0, 0, 1, 1, 0, 0, 1]$ (period 8).
...
At any snapshot $\mathbf{W}$, plasticity is frozen ($\eta_0 = 0$). The network is probed with pattern $X \in \{A, B, C\}$ for $T_{\text{drive}} = 32$ ticks, followed by relaxation for $T_{\text{relax}} = 64$ ticks.
...
4. **Retained Multistable Attractor Separation ($D_{\text{sep}}$):**
   ...
   $$D_{\text{sep}}(A, B, C \mid \mathbf{W}^{(C)}) = \min_{(X, Y) \in \{(A,B), (B,C), (A,C)\}} D_{\text{sep}}(X, Y \mid \mathbf{W}^{(C)})$$
5. **Subspace Engram Overlap ($\mathcal{O}$):**
   Let $\Delta \mathbf{W}_X = \mathbf{W}^{(X)} - \mathbf{W}^{(0)}$.
   $$\mathcal{O}(X, Y) = \frac{\sum_{i=1}^N \sum_{j \in \mathcal{N}^{\text{in}}(i)} \Delta W_{X, ij} \Delta W_{Y, ij}}{\|\Delta \mathbf{W}_X\|_F \|\Delta \mathbf{W}_Y\|_F}$$
