...
$$\mathcal{V} = \big\{ (x, y) \mid x, y \in \mathbb{Z}_4 \big\} \cong \{0, 1, \dots, 15\}$$
...
$$\mathcal{N}_{(x,y)} = \big\{ ((x+1)\bmod 4, y), \, ((x-1)\bmod 4, y), \, (x, (y+1)\bmod 4), \, (x, (y-1)\bmod 4) \big\}$$
...
- $s_i(t) \in \{0, 1\}$: Binary spike emission at tick $t$.
- $V_i(t) \in \mathbb{R}_{\ge 0}$: Internal membrane accumulator potential (charge counter).
- $R_i(t) \in \{0, 1, \dots, N_{\text{ref}}\}$: Refractory cooldown counter.
- $H_i(t) \in \{0, 1\}^k$: Local $k$-bit ring buffer temporal history ($k = 8$).
...
The incoming track vector to node $i$ at tick $t$ is denoted $\mathbf{u}_i(t) = \big( b_{j \to i}(t) \big)_{j \in \mathcal{N}_i} \in \{0, 1\}^4$.

### 3. Update Operators

The transition function $\Phi: \mathcal{S}^N \to \mathcal{S}^N$ is composed of four deterministic operators applied synchronously:

1. **Incoming Logic Combinator $\mathcal{C}$**: Maps incoming track bits to instantaneous injected charge $c_i(t) = \sum_{j \in \mathcal{N}_i} b_{j \to i}(t) \in \{0, 1, 2, 3, 4\}$.
...
| Refractory Period | $N_{\text{ref}}$ | $\{1, 2, 3\}$ ticks | Absolute recovery deadtime |
...
| Sliding Window | $W_h$ | $\{50, 100, 200\}$ ticks | Temporal rate integration window |
...
$$H_i(t+1) = \big( s_i(t), \, H_{i, 0}(t), \, H_{i, 1}(t), \, \dots, \, H_{i, k-2}(t) \big) \in \{0, 1\}^k$$
...
Throughout the evolution of system trajectory $\mathcal{T} = \{ S_i(t) \}_{i \in \mathcal{V}, t \ge 0}$, the following invariants hold strictly:
...
1. **Absorbing Extinction Failure:** Active homeostatic runs suffer complete state collapse (all-zeros $\mathbf{s}_i(t) = 0$ for all $i$ sustained for $L_{\text{ext}} = 100$ consecutive ticks) in $\ge 5\%$ of tested seeds ($M \ge 30$) under nominal settings ($\lambda \in [0.05, 0.10], N_{\text{ref}} \in \{1, 2\}, r_{\text{target}} \in [0.08, 0.16]$).
