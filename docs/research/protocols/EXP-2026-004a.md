...
1. **Plasticity Learning Rate ($\eta_{\text{hebb}}$):** $\eta_{\text{hebb}} \in \lbrace 0.005, 0.01, 0.02 \rbrace$. Canonical default: $\eta_{\text{hebb}} = 0.01$.
2. **Training Exposure Duration ($T_{\text{hebb}}$):** $T_{\text{hebb}} \in \lbrace 500, 1000, 2000 \rbrace$ discrete clock ticks. Canonical default: $T_{\text{hebb}} = 1000$.
3. **Driving Pattern ($P$):** Four orthogonal 32-tick patterns $P \in \lbrace A, B, C, D \rbrace$.
4. **Test Bit-Flip Noise Level ($\epsilon$):** $\epsilon \in \lbrace 0.00, 0.01, 0.05 \rbrace$.
5. **Experimental Condition:**
   ...
   - `novel_pattern_control`: Network exposed to $P_{\text{train}}$, tested on novel unexposed patterns $P_{\text{novel}} \in \lbrace A, B, C, D \rbrace \setminus \lbrace P_{\text{train}} \rbrace$.
6. **Random Seed ($S$):** 30 independent pseudo-random seeds ($S \in \lbrace 42, 43, \dots, 71 \rbrace$).
...
1. **Intra-Pattern Replay Distance ($D_{\text{replay}}$):**
   $$D_{\text{replay}}(P) = \frac{1}{N} \lVert \mathbf{m}_P^{(1)} - \mathbf{m}_P^{(2)} \rVert_1 \in [0.0, 1.0]$$
2. **Inter-Pattern Minimum Separation ($D_{\text{sep}}$):**
   $$D_{\text{sep}}(P) = \min_{Q \neq P} \frac{1}{N} \lVert \mathbf{m}_P - \mathbf{m}_Q \rVert_1 \in [0.0, 1.0]$$
...
4. **Novel Pattern Control (`novel_pattern_control`):** Network trained on $P_{\text{train}} = A$ is evaluated on $P_{\text{novel}} \in \lbrace B, C, D \rbrace$. Tests whether basin deepening is associative and pattern-specific versus non-specific global excitability tuning.
...
| `plasticity_rate` | Float | $0.01$ | $\lbrace 0.005, 0.01, 0.02 \rbrace$ | Hebbian learning rate sweep |
| `exposure_ticks` | Integer | $1000$ | $\lbrace 500, 1000, 2000 \rbrace$ | Training exposure sweep |
| `noise_level` | Float | $0.05$ | $\lbrace 0.00, 0.01, 0.05 \rbrace$ | Perturbation robustness test |
