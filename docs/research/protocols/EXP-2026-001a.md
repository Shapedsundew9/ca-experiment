...
1. **Experimental Mechanism Mode ($M_{\text{mode}}$):**
   ...
   - `random_drift`: Decoupled stochastic threshold drift:
     ...
     $$\Delta V_i(t) = \eta \cdot \xi_i(t), \quad \xi_i(t) \in \lbrace -1, +1 \rbrace \text{ with equal probability } 0.5$$

   - `inverted`: Positive feedback anti-homeostasis:

     $$\Delta V_i(t) = -\eta \cdot (\bar{r}_i(t) - r_{\text{target}})$$

2. **Accumulator Leak Factor ($\lambda$):** $\lambda \in \lbrace 0.00, 0.05, 0.10, 0.20 \rbrace$ tick$^{-1}$.
3. **Refractory Cooldown Period ($N_{\text{ref}}$):** $N_{\text{ref}} \in \lbrace 1, 2, 3 \rbrace$ ticks.
4. **Target Firing Setpoint ($r_{\text{target}}$):** $r_{\text{target}} \in \lbrace 0.08, 0.12, 0.16 \rbrace$.
5. **Adaptation Learning Rate ($\eta$):** $\eta \in \lbrace 0.005, 0.020, 0.050 \rbrace$ (for `active`, `random_drift`, `inverted`).
6. **Sliding Integration Window ($W_h$):** $W_h \in \lbrace 50, 100, 200 \rbrace$ ticks.
7. **Random Seed ($\mathcal{S}$):** $\mathcal{S} \in \lbrace 1, 2, \dots, 30 \rbrace$ ($30$ independent deterministic runs per condition).
...
1. **Survival Duration ($\tau_{\text{survive}}$):**
   ...
   $$\tau_{\text{survive}} = \min \Big\lbrace t \in [1, T] \ \Big|\ \sum_{\tau=0}^{99} \bar{\rho}(t-\tau) = 0 \Big\rbrace \quad (\text{or } T \text{ if no extinction occurs})$$
...
1. **Baseline Control (`fixed`):** Sets $\eta = 0.0$. Initial thresholds are fixed across the run at $V_{\text{thresh}}^{(0)} \in \lbrace 1.5, 2.5, 3.5 \rbrace$. Demonstrates the inability of unadapted discrete CA networks to avoid extinction or saturation under non-trivial leaks.
...
| :--- | :--- | :--- | :--- |
| **P1: Zero Extinction** | $P_{\text{ext}}$ on `active` condition | Exactly $0.0$ ($0$ of $30$ seeds) for $\lambda \in \lbrace 0.05, 0.10 \rbrace$ | **FAIL (REFUTE HYP)** |
...
| **P3: Firing Confinement** | Steady-state density $\langle \bar{\rho} \rangle$ | $\langle \bar{\rho} \rangle \in [0.05, 0.20]$ and $\lVert \langle \bar{\rho} \rangle - r_{\text{target}} \rVert \le 0.03$ | **FAIL (REFUTE HYP)** |
