...
1. **Delay Horizon ($\tau$):** $\tau \in \{3, 5, 7, 10\}$ discrete clock ticks.
2. **Input Pulse Density ($p_x$):** Bernoulli probability $p_x \in \{0.10, 0.20, 0.50\}$. Default canonical value: $p_x = 0.20$.
...
4. **Ridge Regularization Parameter ($\alpha_{\text{ridge}}$):** $\alpha_{\text{ridge}} \in \{10^{-4}, 10^{-2}, 1.0\}$. Default: $10^{-2}$.
5. **Random Seed ($S$):** 30 independent seeds ($S \in \{42, 43, \dots, 71\}$).
...
5. **Readout Weight Norm ($\|\mathbf{w}\|_2$):** L2-norm of the learned linear classification vector.
...
3. **Confidence Intervals:** 95% bootstrap confidence intervals (10,000 resamples) for mean accuracy at each delay $\tau \in \{3, 5, 7, 10\}$.
...
| `plasticity_rate` | Float | $0.02$ | $\{0.02, 0.00\}$ | Active homeostasis vs static ablation |
| `sensory_node` | Integer | $0$ | Fixed port $0$ | Ingress injection location |
| `delay_tau` | Integer | $5$ | $\{3, 5, 7, 10\}$ | Primary independent variable |
| `pulse_density` | Float | $0.20$ | $\{0.10, 0.20, 0.50\}$ | Input sparsity level |
| `ridge_alpha` | Float | $0.01$ | $\{10^{-4}, 10^{-2}, 1.0\}$ | Regularization stability |
